...
Файл ```scripts.py``` содержит функции, которые могут: исправлять оценки, удалять замечания и отправлять хваленые сообщения.
Для их запуска нужно использовать интерактивный режим Django shell
```
python3 manage.py shell
```
Затем подключить скрипты и запустить необходимую для вас функцию
```
fix_marks(schoolkid)
```
